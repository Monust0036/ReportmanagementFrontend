...
class LeaveApplicationEmpTable extends Component {
  state = {
    ...
    columnDefs: [
      ...
      {
        headerName: "TourStartLocation",
        field: "TourStartLocation",
        sortable: true,
        // width: 150,
        // filter: true ,
      },
      ...
      // },
      {
        headerName: "DestinationPlanned",
        field: "DestinationPlanned",
        sortable: true,
        // width: 150,
        // filter: true ,
      },
      {
        headerName: "TourNoGenerated",
  ...
  loadLeaveApplicationEmpData = () => {
    axios
      .get(
        process.env.REACT_APP_API_URL+ "/api/tour-detail-emp/" +
        this.props.data["_id"], {
        headers: {
      ...
      .then(response => {
        ...
        this.leaveApplicationEmpObj.tourDetail.map(data => {
          let temp = {
            data,
            TeamLeaderName: data["TeamLeaderName"],
            TeamMember: data["TeamMember"],
            TourStartLocation: data["TourStartLocation"],
            TourStartDate: data["TourStartDate"].slice(0, 10),
            DestinationPlanned:data["DestinationPlanned"],
  ...
  onLeaveApplicationEmpDelete = (e1, e2) => {
    ...
    if (window.confirm("Are you sure to delete this record? ") == true) {
      axios
        .delete(
          process.env.REACT_APP_API_URL+ "/api/tour-detail-emp/" + e1 + "/" + e2, {
          headers: {
            authorization: localStorage.getItem("token") || ""
          }
        }
  ...
  render() {
    return (
      <div id="table-outer-div-scroll">
        <h2 id="role-title">Tour Detail</h2>
